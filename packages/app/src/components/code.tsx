...
function transformerUnifiedDiff(): ShikiTransformer {
  ...
  return {
    ...
    line(node, line) {
      ...
      if (m.old != null) (node.properties as any)["data-old"] = String(m.old)
      if (m.new != null) (node.properties as any)["data-new"] = String(m.new)

      const oldSpan = {
        type: "element",
        tagName: "span",
        properties: { className: ["diff-oldln"] },
        children: [{ type: "text", value: m.old != null ? String(m.old) : " " }],
      }
      const newSpan = {
        type: "element",
        tagName: "span",
        properties: { className: ["diff-newln"] },
        children: [{ type: "text", value: m.new != null ? String(m.new) : " " }],
      }
...
function applySplitDiff(container: HTMLElement) {
  ...
  const originalCode = originalPre.querySelector("code") as HTMLElement | null
...
export function Code(props: Props) {
  ...
  const [html] = createResource(
    ...
    async ([code, lang]) => {
      return (await codeToHtml(code || "", {
        ...
        transformers: [transformerUnifiedDiff(), transformerNotationDiff()], //, transformerRenderWhitespace()],
      })) as string
    },
  )
  ...
  // Fold long runs of unchanged context lines (unified view only)
  createEffect(() => {
    const content = html()
    if (!container || !content) return
    const path = container.getAttribute("data-source-file") || ""
    if (!path) return
    const view = ctx.file.view(path)
    if (view === "diff-split") return
    const code = container.querySelector("pre code") as HTMLElement | null
    if (!code) return
    const expanded = path ? ctx.file.folded(path) : []
    applyDiffFolding(code, 3, {
      expanded,
      onExpand: (key) => {
        if (path) ctx.file.unfold(path, key)
      },
    })
  })

  // Build or toggle split diff layout based on LocalFile state
  createEffect(() => {
    const content = html()
    if (!container || !content) return
    const path = container.getAttribute("data-source-file") || ""
  ...
  // Fold unchanged context lines in split view (both panes stay in sync)
  createEffect(() => {
    const content = html()
    if (!container || !content) return
    const path = container.getAttribute("data-source-file") || ""
    ...
    const left = container.querySelector<HTMLDivElement>(".diff-split pre:nth-child(1) code")
    const right = container.querySelector<HTMLDivElement>(".diff-split pre:nth-child(2) code")
    if (!left || !right) return

    applyDiffFolding(left, 3, {
      expanded,
      onExpand: (key) => {
        if (path) ctx.file.unfold(path, key)
      },
